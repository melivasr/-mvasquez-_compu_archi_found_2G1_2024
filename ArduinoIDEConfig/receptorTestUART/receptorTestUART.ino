...
void loop() {
  ...
  if (esperandoLiberacion && estadoBotonAnterior == LOW && estadoBotonActual == HIGH) {
    ...
    // Enviar el byte al emisor
    Serial1.write(byteEnviado);

    // Imprimir mensaje al monitor serial
    Serial.print("Receptor: Enviando byte ");
    Serial.print(byteEnviado, BIN);
    Serial.println(" al emisor");
